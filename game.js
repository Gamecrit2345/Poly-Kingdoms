...
// Rich Terrain Generation
for (let i = 0; i < 1400; i++) {
    map[Math.floor(Math.random()*WORLD_H)][Math.floor(Math.random()*WORLD_W)] = 1; // Trees
}
for (let i = 0; i < 520; i++) {
    const rx = Math.floor(Math.random()*WORLD_W);
    const ry = Math.floor(Math.random()*WORLD_H);
    if (map[ry][rx] === 0) map[ry][rx] = 2; // Hills
}
for (let i = 0; i < 320; i++) {
    const rx = Math.floor(Math.random()*WORLD_W);
    const ry = Math.floor(Math.random()*WORLD_H);
    if (map[ry][rx] === 0) map[ry][rx] = 3; // Water
}

const philippineKingdoms = [
    "San Jose del Monte Kingdom", "Sapang Palay Kingdom", "Quezon City Empire", 
    "Manila Kingdom", "Davao Empire", "Cebu Kingdom", "Cagayan de Oro Realm",
    "Zamboanga Sultanate", "Antipolo Kingdom", "Baguio Highland Kingdom",
    "Iloilo Kingdom", "General Santos Realm", "Makati Empire", 
    "Taguig Kingdom", "Pasig River Kingdom"
];
...
function draw() {
    ...
    for (let y = startY; y < endY; y++) {
        for (let x = startX; x < endX; x++) {
            ...
            } else if (type === 2) { // Hills
                ctx.fillStyle = '#264726'; ctx.fillRect(sx+2, sy+2, GRID-4, GRID-4);
                ctx.fillStyle = '#3f5f3f'; 
                ctx.beginPath();
                ctx.ellipse(sx + GRID/2, sy + GRID/2.3, GRID*0.42, GRID*0.25, 0, 0, Math.PI*2);
                ctx.fill();
                ctx.fillStyle = 'rgba(0,0,0,0.4)';
                ctx.fillRect(sx+8, sy+GRID-8, GRID-12, 6);
            } else if (type === 1) { // Trees
                ctx.fillStyle = '#0f2a0f'; 
                ctx.fillRect(sx + GRID*0.32, sy + GRID*0.58, GRID*0.36, GRID*0.38);
                ctx.fillStyle = '#094';
                ctx.beginPath(); 
                ctx.arc(sx + GRID/2, sy + GRID/3.1, 21, 0, Math.PI*2); 
                ctx.fill();
                ctx.fillStyle = '#1b9';
                ctx.beginPath(); 
                ctx.arc(sx + GRID/2 - 7, sy + GRID/3.6, 14, 0, Math.PI*2); 
                ctx.fill();
            }
        }
    }
...
canvas.addEventListener('mousedown', e => { 
    if(!placing){ isDragging = true; lastX = e.clientX; lastY = e.clientY; }
});
...
canvas.addEventListener('touchstart', e => { 
    if(!placing){ isDragging = true; lastX = e.touches[0].clientX; lastY = e.touches[0].clientY; }
}, {passive:false});
...
function attackBarbarians() {
    ...
    if(win){
        resources.gold += loot; 
        resources.food += Math.floor(loot*0.7); 
        troops = Math.max(6, troops-4); 
        log(`🏆 Nanalo! +${loot} ginto`);
    } else {
        troops = Math.max(3, troops-10); 
        log("💥 Natalo sa laban...");
    }
    updateUI();
}
...
setInterval(() => {
    ...
    resources.wood += halls * 7;
    resources.stone += halls * 5;
    updateUI();
}, 2400);
